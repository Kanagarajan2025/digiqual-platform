...
public class DashboardService {
    ...
    public void updateCertificateStage(String email, String certificateCode, String stage) {
        ...
        CertificateRecord certificate = certificateRecordRepository.findByCertificateCode(certificateCode)
                ...
                if ("issued".equalsIgnoreCase(normalizedStage) && !certificate.isPaymentConfirmed()) {
                        throw new IllegalArgumentException("Payment must be confirmed before issuing certificate");
                }

                certificate.setStage(normalizedStage);
                if ("issued".equalsIgnoreCase(normalizedStage)) {
                        certificate.setIssuedAt(LocalDateTime.now());
                }
        certificateRecordRepository.save(certificate);
    }

        public void updateCertificatePayment(String email, String certificateCode, boolean paid) {
                User user = getUserByEmail(email);
                requireRole(user, User.UserRole.ADMIN, "Admin access required");

                CertificateRecord certificate = certificateRecordRepository.findByCertificateCode(certificateCode)
                                .orElseThrow(() -> new IllegalArgumentException("Certificate not found"));
                certificate.setPaymentConfirmed(paid);
                certificateRecordRepository.save(certificate);
        }
        ...
        public void approveStudent(String email, Long enrollmentId) {
                provisionStudentLogin(enrollmentId);
    ...
    private void requireRole(User user, User.UserRole role, String message) {
        ...
        private void provisionStudentLogin(Long enrollmentId) {
                StudentEnrollment enrollment = studentEnrollmentRepository.findById(enrollmentId)
                                .orElseThrow(() -> new IllegalArgumentException("Enrollment not found"));

                String normalizedEmail = enrollment.getEmail() == null
                                ? ""
                                : enrollment.getEmail().trim().toLowerCase(Locale.ROOT);

                if (normalizedEmail.isBlank()) {
                        throw new IllegalArgumentException("Student email is required for login provisioning");
                }

                User studentUser = userRepository.findByEmailAndRole(normalizedEmail, User.UserRole.STUDENT)
                                .orElseGet(() -> {
                                        User user = new User();
                                        user.setEmail(normalizedEmail);
                                        user.setRole(User.UserRole.STUDENT);
                                        user.setFullName(enrollment.getFullName());
                                        user.setActive(true);
                ...
                studentUser.setLastLogin(null);
                User saved = userRepository.save(studentUser);

                enrollment.setUserAccount(saved);
                studentEnrollmentRepository.save(enrollment);

                String studentId = String.format(Locale.ROOT, "DGQ-%d-%03d", saved.getCreatedAt().getYear(), saved.getId());
                notificationService.sendStudentLoginCredentials(normalizedEmail, enrollment.getFullName(), studentId, tempPassword);
        }

        private String buildTemporaryPassword() {
                String raw = UUID.randomUUID().toString().replace("-", "");
                return "Dgq@" + raw.substring(0, 8);
        }
